...
int snd_client(int sock, struct sockaddr_in * serveur, s_MUV* packetS)
{
	...
	if((nbS = sendto(sock, packetS, SIZE_PACKET + sizeof(long), 0, (struct sockaddr *) serveur, sizeof(struct sockaddr_in) )) > 0)
...
int rcv_client(int sock, s_MUV* packetR)
{
	...
	if ((nbR = recv(sock, packetR,SIZE_PACKET + sizeof(long), MSG_DONTWAIT)) <= 0)
		return EXIT_FAILURE;
	else 
		printf("berf3\n");
